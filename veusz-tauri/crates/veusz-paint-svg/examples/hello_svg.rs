//! Counterpart to `veusz-paint-pdf/examples/hello_pdf.rs`: build the same
//! scene, emit an SVG.
//!
//! Run:
//!   cargo run --release -p veusz-paint-svg --example hello_svg -- /tmp/hello.svg
...
fn build_scene() -> Scene {
    ...
    // Gradient (lowered to first-stop colour for now, same as PDF emitter).
    rec.set_paint(&Paint {
    ...
    // Text (rendered as glyph paths via veusz-paint-text when fonts are
    // available; otherwise falls back to a dashed bounding-box placeholder).
    rec.draw_text(
        &TextLayout {
            text: "veusz-paint-svg (text via glyph paths)".into(),
...
fn main() {
    ...
    let svg = render_scene_to_svg(&scene, 400.0, 240.0, (1.0, 1.0, 1.0, 1.0)).expect("emit");
    let out_path = std::env::args().nth(1).unwrap_or_else(|| "/tmp/hello.svg".into());
    let mut f = File::create(&out_path).expect("open");
    f.write_all(&svg).expect("write");
    println!("wrote {} bytes to {}", svg.len(), out_path);
}
